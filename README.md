...
一款面向微信小程序开发者的学习工具，系统展示 42+ 个组件及摄像头、麦克风、陀螺仪、定位等设备能力的实际效果、属性说明、示例代码和使用提示。完整源码，可商用。

## 在线教程

本项目的配套图文教程（开发思路、组件速查、部署手册、协作复盘）已部署为在线文档站：

👉 **[微信小程序教程 · vibe coding 版](https://flyingwenku.github.io/wechat-miniprogram-tutorial/#/)**
...
所有组件数据集中在 `data/controls.js`，包含：
- 组件 ID、名称、描述
- 属性列表（name / type / default / desc）
- 示例代码（wxml / js / wxss）
- 使用提示

新增组件只需在数据文件中添加一条记录，并在对应的 demo 组件中实现演示逻辑。
...
- 详情页根据组件的 `categoryId` 加载对应的演示组件

## 扩展指南

### 新增一个组件

1. 在 `data/controls.js` 对应分类的 `controls` 数组中添加组件数据
